...
with sqlite3.connect('vkim.sqlite') as db:
 cursor = db.cursor()
 query = """ SELECT messages.local_id, messages.vk_id, messages.cnv_msg_id, users.first_name_nom, users.last_name_nom, messages.dialog_id, messages.time, messages.from_member_type, messages.from_member_id, messages.is_incoming, messages.body, messages.attach, messages.nested FROM messages, users WHERE messages.dialog_id = users.id """
...
excel_file = xlsxwriter.Workbook('best_selection_for_private.xlsx')
...
excel_worksheet.write(0,3,'Имя собеседника',font_for_headers)
excel_worksheet.write(0,4,'Фамилия собеседника',font_for_headers)
excel_worksheet.write(0,5,'ID собеседника',font_for_headers)
excel_worksheet.write(0,6,'Дата и время (UTC)',font_for_headers)
excel_worksheet.write(0,7,'Тип отправителя (1 - пользователь, 3 - группа/канал)',font_for_headers)
excel_worksheet.write(0,8,'ID отправителя',font_for_headers)
excel_worksheet.write(0,9,'Входящее ? (1 - да, 0 - нет)',font_for_headers)
excel_worksheet.write(0,10,'Текст сообщения',font_for_headers)
excel_worksheet.write(0,11,'Интернет-ссылка на медиа-сообщение',font_for_headers)
excel_worksheet.write(0,12,'Интернет-ссылка на медиа-сообщение, на которое пользователь ответил',font_for_headers)
...
for string_from_query in tuple_from_query:
 ...
 excel_worksheet.write(row_counter,5,str(string_from_query[5]))
 not_converted_time = string_from_query[6]
 converted_time = datetime.datetime.utcfromtimestamp(int(str(not_converted_time)[0:10]))
 excel_worksheet.write(row_counter,6,str(converted_time))
 ...
 excel_worksheet.write(row_counter,10,str(string_from_query[10]))
...
# Осуществляем поиск в строках полей "attach", "nested" подстрок со ссылками на голосовые сообщения, либо на изображения и записываем результат в Excel-файл построчно
 results_image_field_attach = re.findall(r'(https.{211,216}album)',str(string_from_query[11]))
 audio_message_attach = re.findall(r'(https.*\.ogg)|(https.*\.mp3)',str(string_from_query[11]))
 ...
 if len(results_image_field_attach) != 0 :
  ...
  excel_worksheet.write(row_counter,11,results_audio_field_attach)
 else :
  excel_worksheet.write(row_counter,11,'')

 results_image_field_nested = re.findall(r'(https.{211,216}album)',str(string_from_query[12]))
 audio_message_nested = re.findall(r'(https.*\.ogg)|(https.*\.mp3)',str(string_from_query[12]))
 ogg_way = ''
 mp3_way = ''
 image_way = ''
 if len(results_image_field_nested) != 0 :
  ...
  while i < len(results_image_field_nested) :
   ...
   image_way = image_way + '\n'
   i += 1 
  excel_worksheet.write(row_counter,12,str(image_way))
  ...
  excel_worksheet.write(row_counter,12,results_audio_field_nested)
 else :
  excel_worksheet.write(row_counter,12,'')
 row_counter += 1
